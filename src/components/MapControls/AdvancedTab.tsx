...
type AirportType = {
    key: "large" | "medium" | "small" | "heliport" | "seaplane" | "closed" | "balloonport";
    label: string;
    color: string;
    size: number;
    icon: string;
...
export default function AdvancedTab({
    ...
    const handleAirportToggle = (type: keyof typeof showAirports) => {
        if (setLoadingStates) {
            setLoadingStates(prev => ({ ...prev, [`airport-${type}`]: true }));
        }
        onToggleAirports({ ...showAirports, [type]: !showAirports[type] });
    };

    const handle3DToggle = (type: "terrain", currentValue: boolean, onToggle: (value: boolean) => void) => {
        if (setLoadingStates) {
    ...
    const handleSelectAllAirports = () => {
        if (setLoadingStates) {
            AIRPORT_TYPES.forEach(airport => {
                setLoadingStates(prev => ({ ...prev, [`airport-${airport.key}`]: true }));
            });
        }
        onToggleAirports({
            large: true, medium: true, small: true, heliport: true,
            seaplane: true, closed: true, balloonport: true,
        });
    };
    ...
    const handleClearAllAirports = () => {
        ...
        onToggleAirports({
            large: false, medium: false, small: false, heliport: false,
            seaplane: false, closed: false, balloonport: false,
        });
    };
            ...
            {onToggleTerrain && (
                <div className="section">
                    ...
                    <div className="layer-list">
                        <button
                            className={`layer-item ${showTerrain ? "active" : ""} ${loadingStates['terrain'] ? "loading" : ""}`}
                            onClick={() => handle3DToggle("terrain", !!showTerrain, onToggleTerrain)}
                            disabled={loadingStates['terrain']}
                            style={{ "--layer-color": "#10b981" } as React.CSSProperties}
                        >
                            <span className="layer-icon">⛰️</span>
                            <span className="layer-label">3D Terrain</span>
                            <div className={`layer-checkbox ${showTerrain ? "active" : ""}`}>
                                {loadingStates['terrain'] ? (
                                    <span className="spinner"></span>
                                ) : (
                                    showTerrain && <span className="checkmark">✓</span>
                                )}
                            </div>
                        </button>
                    </div>
            ...
            <div className="section">
                <div className="section-header">
                    ...
                    <div className="section-controls">
                        <button onClick={handleSelectAllAirports} className="control-button primary">
                            All
                        </button>
                        <button onClick={handleClearAllAirports} className="control-button outline">
                            None
                        </button>
                    </div>
                </div>
                ...
                <div className="card-grid">
                    {AIRPORT_TYPES.map((airport) => {
                        ...
                        const isLoading = loadingStates ? loadingStates[`airport-${airport.key}`] || false : false;
                        return (
                            <button
                                key={airport.key}
                                className={`filter-card ${isActive ? "active" : ""} ${isLoading ? "loading" : ""}`}
